...
void timeKeeping() {
    ...
    while (1) {
        ...
        // ======= KIEM TRA KHONG DUOC CHAM CONG TRUOC TUONG LAI =======
        if (y > today_y ||
           (y == today_y && m > today_m) ||
...
void viewTimeSheet() {
    ...
    while (1) {
        ...
        if (month < 1 || month > 12) {
            printf("Thang phai tu 1 den 12!\n");
            continue;
        }
        break;
    }

    // ====== KHONG CHO XEM TUONG LAI ======
    ...
    // ====== CHI IN DEN NGAY HIEN TAI ======
    int displayDays = totalDays;
    if (year == today.tm_year + 1900 && month == today.tm_mon + 1) {
        displayDays = today.tm_mday;
    }

    printf("\n---- Bang cham cong cua nhan vien co ID: %s - Thang %02d/%04d ----\n", viewEmpId, month, year);
    printf("ID Log     | Ngay        | Trang thai\n");
    printf("----------------------------------------\n");

    // In tu ngay 1 den ngay hien
    char dateCheck[20];
    int found;

    for (day = 1; day <= displayDays; day++) {

        sprintf(dateCheck, "%02d/%02d/%04d", day, month, year);
        found = 0;

        for (i = 0; i < logCount; i++) {
            if (strcmp(logs[i].empId, viewEmpId) == 0 &&
                strcmp(logs[i].date, dateCheck) == 0) {

                printf("%-10s | %-10s | %s\n",
                       logs[i].logId, logs[i].date, logs[i].status);
                found = 1;
                break;
            }
        }
